...
<div class="content">
    ...
    <div class="cardView">
        <div class="cards" :style='{"margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
			<div :style='{"boxShadow":"0 1px 6px rgba(0,0,0,.3)","margin":"0 10px","borderRadius":"4px","display":"flex"}' v-if="isAuth('tushuxinxi','首页总数')">
        ...
        <div style="display: flex;align-items: center;width: 100%;margin-bottom: 10px;">
            <el-card style="width: 50%;margin: 0 10px;" v-if="isAuth('tushuxinxi','首页统计')">
                <div id="tushuxinxiChart1" style="width:100%;height:400px;"></div>
            </el-card>
            <el-card style="width: 50%;margin: 0 10px;" v-if="isAuth('tushuxinxi','首页统计')">
                <div id="tushuxinxiChart2" style="width:100%;height:400px;"></div>
            </el-card>
        </div>
    </div>
</div>
  ...
  methods:{
    ...
    tushuxinxiChat2() {
      this.$nextTick(()=>{
        ...
        var tushuxinxiChart2 = echarts.init(document.getElementById("tushuxinxiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                tushuxinxiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    tushuxinxiChart2.resize();
                };
            }
        });
      })
